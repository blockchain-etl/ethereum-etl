...
class RabbitMQItemExporter:
    OUTPUT_FORMAT_ERROR = 'Invalid rabbitmq output param. ' \
                   'It should be in format of "rabbitmq/{username}:{password}@localhost:5672"'
    ...
    def connect(self):
        if not self._connection or self._connection.is_closed:
            ...
            self._channel.queue_declare(queue='etl-queue', durable=True)
