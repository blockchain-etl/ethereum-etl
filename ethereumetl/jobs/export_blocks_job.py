...
class ExportBlocksJob(BatchExportJob):
    def __init__(
        ...
        if not self.export_blocks and not self.export_transactions and not self.export_receipts:
            raise ValueError('At least one of export_blocks or export_transactions or export_receipts must be True')
